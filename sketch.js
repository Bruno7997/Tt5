...
for (var i=0;i<m.length;i++){
  ...
  else if(Math.sqrt((Math.pow(mpx[1],2)+Math.pow(mpy[1],2)))<Math.sqrt((Math.pow(mpx[2],2)+Math.pow(mpy[2],2)))){m[i].speed(p[player.index-1].position.x,p[player.index-1].position.y,i)}
  else {m[i].speed(p[Math.round(random(0,playerCount-1))].position.x,p[Math.round(random(0,playerCount-1))].position.y,i)}
...
if(m.length==0){indexL=0;indexm=0;
  ...
  if (gameState === 0||gameState==1){if(game.r==0){Monster.getMonstersInfo();game.createMonster()};game.play()
    //Pontuação
...
if(m.length==0&&keyCode==32&&gameState==1){;qdm+=(qdm/(Round*0.95));game.update(0)}
  ...
  //W:
if(keyCode===87&&X!=1){setTimeout(()=>{if(player.index!=null){var bul = new Bullet(p[player.index-1].position.x,p[player.index-1].position.y,0,-vdb,d,player.index,1);buls.push(bul)}},500)}
  //S:
if(keyCode===83&&X!=1){setTimeout(()=>{if(player.index!=null){var bul = new Bullet(p[player.index-1].position.x,p[player.index-1].position.y,0,vdb,d,player.index,1);buls.push(bul)}},500)}
  //A:
if(keyCode===65&&X!=1){setTimeout(()=>{if(player.index!=null){var bul = new Bullet(p[player.index-1].position.x,p[player.index-1].position.y,-vdb,0,d,player.index,1);buls.push(bul)}},500)}
  //D:
if(keyCode===68&&X!=1){setTimeout(()=>{if(player.index!=null){var bul = new Bullet(p[player.index-1].position.x,p[player.index-1].position.y,vdb,0,d,player.index,1);buls.push(bul)}},500)}

//Contrario do Relogio
//W:
if(keyCode===87&&X==1){setTimeout(()=>{if(player.index!=null){var bul = new Bullet(p[player.index-1].position.x,p[player.index-1].position.y,-vdb,-vdb,d,player.index,1);buls.push(bul)}},500)}
  //S:
if(keyCode===83&&X==1){setTimeout(()=>{if(player.index!=null){var bul = new Bullet(p[player.index-1].position.x,p[player.index-1].position.y,vdb,vdb,d,player.index,1);buls.push(bul)}},500)}
  //A:
if(keyCode===65&&X==1){setTimeout(()=>{if(player.index!=null){var bul = new Bullet(p[player.index-1].position.x,p[player.index-1].position.y,-vdb,vdb,d,player.index,1);buls.push(bul)}},500)}
  //D:
if(keyCode===68&&X==1){setTimeout(()=>{if(player.index!=null){var bul = new Bullet(p[player.index-1].position.x,p[player.index-1].position.y,vdb,-vdb,d,player.index,1);buls.push(bul)}},500)}
}
